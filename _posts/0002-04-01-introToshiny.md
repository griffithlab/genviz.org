...
To make your new shiny app accessible on the web you have several options. The simplest is to just sign up for an account at [www.shinyapps.io](http://www.shinyapps.io/). Once you sign up shinyapps.io will walk you through the process of installing and setting up the rsconnect library (just two commands). If set up correctly you will be able to simply select the 'Publish' button in the top-right of a running Shiny App from Rstudio. This will create an app at https://[your_account.shinyapps.io/yourApp/
