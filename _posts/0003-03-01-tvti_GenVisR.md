...
Up til this point we have glossed over what [arrangeGrob()](https://www.rdocumentation.org/packages/gridExtra/versions/2.2.1/topics/arrangeGrob) is actually doing however this is no longer a luxury we can afford. In the previous figure we can see that adding the clinical data to our plot has caused our proportion and frequency plots to go out of alignment. This has occurred because the y-axis text in the clinical plot takes up more space than in the frequency plot. Because we added clinical data with [GenVisR](https://bioconductor.org/packages/release/bioc/html/GenVisR.html) to the proportion plot those graphics are aligned with each other however [TvTi()](https://www.rdocumentation.org/packages/GenVisR/versions/1.0.4/topics/TvTi) has no idea the frequency plot exists, it's added to our graphic after the fact with [arrangeGrob()](https://www.rdocumentation.org/packages/gridExtra/versions/2.2.1/topics/arrangeGrob). In order to fix this issue we will need a basic understanding of "grobs", "TableGrobs", and "viewports". First off a [grob](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) is just short for "grid graphical object" from the low-level graphics package [grid](https://www.rdocumentation.org/packages/grid/versions/3.4.1); Think of it as a set of instructions for create a graphical object (i.e. a plot). The graphics library underneath all of [ggplot2's](http://ggplot2.tidyverse.org/reference/) graphical elements are really composed of [grob's](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) because ggplot2 uses [grid](https://www.rdocumentation.org/packages/grid/versions/3.4.1) underneath. A  TableGrob is a class from the [gtable](https://www.rdocumentation.org/packages/gtable/versions/0.2.0) package and provides an easier way to view and manipulate groups of [grobs](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob), it is actually the intermediary between [ggplot2](http://ggplot2.tidyverse.org/reference/) and [grid](https://www.rdocumentation.org/packages/grid/versions/3.4.1). A "viewport" is a graphics region for which a [grob](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) is assigned. We have already been telling [GenVisR](https://bioconductor.org/packages/release/bioc/html/GenVisR.html) to output [grobs](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) instead of drawing a graphic because we have been using the [arrangeGrob()](https://www.rdocumentation.org/packages/gridExtra/versions/2.2.1/topics/arrangeGrob) function in [gridExtra](https://cran.r-project.org/web/packages/gridExtra/index.html) to arrange and display extra viewports in order to create a final plot. Let's briefly look at what this actually means, we can show the layout of viewports with the function [gtable_show_layout()](https://www.rdocumentation.org/packages/gtable/versions/0.2.0/topics/gtable_show_layout). In the figure below we have overlayed this layout on to our plot and have shown the [table grob](https://www.rdocumentation.org/packages/gtable/versions/0.2.0/topics/gtable) to the right. In the [table grob](https://www.rdocumentation.org/packages/gtable/versions/0.2.0/topics/gtable) we can see that at the top level our [grobs](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) are compsed of 2 vertical elements and 1 horizontal elements. The column z corresponds to the order of plot layers, and cells corresponds to the viewport coordinates. So we can see that the [grob](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) listed in row 1 is plotted first and is located on the viewport spanning vertical elemnents 1-1 and horizontal elements 1-1 and thus on our layout is in the viewport (1, 1).
...
Pretty simple right? But not so fast the [table grob](https://www.rdocumentation.org/packages/gtable/versions/0.2.0/topics/gtable) in `finalGrob` is a little over simplified on the surface. It is true that our plot is composed of two final [grobs](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) however each [grob](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) is made up of lists of [grobs](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) in the [table grob](https://www.rdocumentation.org/packages/gtable/versions/0.2.0/topics/gtable) all the way down to the individual base elements which make up the plot. In the figure below we illustrate the second layer of these lists. To re-align the plots we'll need to peel back the layers until we can acess the original [grobs](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) from [ggplot2](http://ggplot2.tidyverse.org/reference/). We will then need to obtain the widths of each of these elements, find the max width with [unit.pmax()](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/unit.pmin), and re-assign the width of each as these [grobs](https://www.rdocumentation.org/packages/grid/versions/3.4.1/topics/grid.grob) using the max width.
